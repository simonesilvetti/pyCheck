...
input_stream = InputStream('F_[30,40] I > 50 &  F_[30,40](R<20) \n')
lexer = STLLexer(input=objectiveInput)
...
for i in range(xx.shape[0]):
    for j in range(xx.shape[1]):
        ...
        # t, sol = sir(xx[i,j], yy[i,j], [95, 5, 0], [0, 300], 500)
        serie = TimeSeries(['A', 'B', 'C'], t, sol.T)
        # visitor = BooleanSemantics(series=serie)
        # zB[i, j] = int(visitor.visit(objectiveTree))
        # serie = StochSerie(['S', 'I', 'R'], t, sol)
        visitor = ZeroSemantics(timeSeries=serie)
        objZeroSemantics[i, j] = visitor.visit(objectiveTree)
        # if np.isnan(objZeroSemantics[i, j]):
        #     objZeroSemantics[i, j]=-2
        visitor = RobSemantics(timeSeries=serie)
        objQuantiativeSemantics[i, j] = visitor.visit(objectiveTree)*2
...
ax1.pcolormesh(xx,yy,objQuantiativeSemantics,vmin=0, vmax=np.max(objQuantiativeSemantics))
# ax1.pcolor(xx,yy,zZ, vmin=np.min(-1), vmax=np.max(1))
ax1.set_title('')
ax2.pcolormesh(xx,yy,objZeroSemantics,vmin=0, vmax=np.max(objZeroSemantics))
