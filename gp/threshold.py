...
def minimizeGP(interval):
    ...
    points = np.linspace(interval[0],interval[1], num=8)
    best = float('Inf')
...
def findMax(matrix):
    # values = [np.max([th-a[0], a[1]-th]) if (a[1]>th and a[0]<th) else -np.min([np.abs(a[1]-th),np.abs(a[0]-th)]) for a in  matrix]
    values = [a[1]-a[0] if (a[1]>th and a[0]<th) else a[0]-a[1] for a in  matrix]
...
plt.plot(x, mmax)
plt.plot(x, mmin)
plt.axhline(y=th, xmin=-1.2, xmax=3.26, hold=None)
plt.show()
