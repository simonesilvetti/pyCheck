...
class FilterSemantics(STLVisitor):
    ...
    def visitU(self, ctx: STLParser.UContext):
        ...
        for i in range(self.currentState,min(indext0, len(s))):
            visitor = FilterSemantics(self.memory, s, i)
        ...
        for i in range(min(indext0, len(s)), min(indext1 + 1, len(s))):
            ...
            sumPositive += np.max([0, maxInner]) * (s.times[i + 1] - s.times[i])
            sumNegative += np.min([0, maxInner]) * (s.times[i + 1] - s.times[i])
        if(sumPositive>0):
            return sumPositive / (indext1 - indext0)
        else:
            return sumNegative / (indext1 - indext0)
    ...
    def visitF(self, ctx: STLParser.FContext):
        ...
        for i in range(self.currentState, indext1):
            value = FilterSemantics(self.memory, s, i).visit(ctx.formula())
            sumPositive+=np.max([0,value])*(s.times[i+1]-s.times[i])
            sumNegative+=np.min([0,value])*(s.times[i+1]-s.times[i])
        if (sumPositive > 0):
            return float(sumPositive / (indext1 - indext0))
        else:
            return float(sumNegative / (indext1 - indext0))
