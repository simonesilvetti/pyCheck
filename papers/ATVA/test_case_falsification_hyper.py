...
from odeModels.insuline.simulation1Day import pidC1,pidC4, simulation, hyperGlicemia
...
for i in range(N):
    ...
    y = y[:, 0]
    stl = kernel(0, t, blowThreshold(p)(y), 0, 1438, 0.9999, flat)
    scl = kernel(0, t, blowThreshold(p)(y), 0, 1438, 0.93, flat)
    if (stl < minSTL):
        minSTL = stl
...
print('SCL ' + str(vSCL))
print('SCL time Violation ' + str(violationTimeS(vSCL, blowThreshold(p), pid)))
print('SCL space Violation ' + str(violationSpaceS(vSCL, blowThreshold(p), pid)))
print('STL ' + str(vSTL))
print('STL time Violation ' + str(violationTimeS(vSTL, blowThreshold(p), pid)))
print('STL space Violation ' + str(violationSpaceS(vSTL, blowThreshold(p), pid)))
